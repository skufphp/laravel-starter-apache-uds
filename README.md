...
*   **Высокая производительность:** Связь между Httpd (Apache) и PHP-FPM реализована через **Unix Socket** в общем томе, что исключает накладные расходы TCP-стека.
*   **Современный стек:**
    *   **PHP 8.5 (Alpine)** — свежая версия с предустановленными расширениями (bcmath, gd, intl, pdo_pgsql и др.).
    *   **Httpd (Apache) 2.4** — оптимизированный конфиг для Laravel с использованием `mod_proxy_fcgi`.
...
*   `make up` — Запустить проект в dev-режиме (на порту `80` или `HTTPD_PORT`).
